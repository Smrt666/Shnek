...
use macroquad::texture::{Texture2D, FilterMode};
...
pub fn load_diffuse(material_path: &Path, material: &Material) -> Texture2D {
    let texture = material.diffuse_texture.clone().expect("no diffuse texture specified");
    ...
    let texture = Texture2D::from_rgba8(image.width() as u16, image.height() as u16, &image.to_rgba8());
    texture
}
